		...
		case CSID.TCP_LOGIN:
			{
				TcpLogin _info = CSData.DeserializeData<TcpLogin> (bodyData);
					int __uid = UserManage.Instance.CheckExistBattle(_info.token);
				if (__uid != -1) // 检查到有未完成的比赛。
				{
						 UserManage.Instance.UpdateUserInfo(_info.token, _socketIp); // 更新IP地址 
						TcpResponseLogin _result = new TcpResponseLogin();
						_result.result = true;
						_result.reconnect = true;
						_result.uid = __uid;
				...
				{
						__uid = UserManage.Instance.UserLogin(_info.token, _socketIp);
						TcpResponseLogin _result = new TcpResponseLogin();
						_result.result = true;
						_result.uid = __uid;
						_result.udpPort = UdpManager.Instance.recvPort;
		...
		case CSID.TCP_REQUEST_MATCH:
			{
				TcpRequestMatch _mes = CSData.DeserializeData<TcpRequestMatch> (bodyData);
					if (_mes.isRecon)  // 重连匹配
					{
						BattleManage.Instance.reconnectClient(_mes.uid);
