	...
	private void RecvThread()
	{
		...
		while (isRecv)
		{
			try {
				...
				byte
					[] bodyData = new byte[bodyDataLenth];
				Array.Copy(buf, PackageConstant.PacketHeadLength, bodyData, 0, bodyDataLenth);
		 	//Debug.LogError(" 收到 packMessageId: " + packMessageId);
				...
				isRecv = false;
				BattleData.Instance.log = "E"+num++;
				Debug.LogError ("udpClient接收数据异常:" + ex.Message+ "    StackTrace:" + ex.StackTrace);
			}
		}
		Debug.Log ("udp接收线程退出~~~~~");
		BattleData.Instance.log = "Q" + num ;
	}
