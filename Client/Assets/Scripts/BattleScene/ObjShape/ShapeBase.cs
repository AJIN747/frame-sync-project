...
public class ShapeBase : MonoBehaviour {
	...
	public virtual bool IsInBaseCircleDistance(GameVector2 _pos,int _radius){
		...
		return ToolGameVector.CollideCircleAndCircle (_pos, basePosition, _radius , baseRadius); // 判断两个圆是否碰撞
	}
	//距离检测,out距离平方
	...
	public virtual bool IsCollisionCircleCorrection(GameVector2 _pos,int _radius,out GameVector2 _amend){
		return ToolGameVector.CollideCircleAndCircle (_pos,basePosition,_radius,baseRadius,out _amend);   // 角色位置，  障碍位置， 角色半径，障碍半径。
	}
}
