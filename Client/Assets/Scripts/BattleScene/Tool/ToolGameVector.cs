...
public class ToolGameVector {
	...
	// 圆和矩形碰撞
	public static bool CollideCircleAndRect(GameVector2 _c1, GameVector2 _c2, int _radius1, int _radius2)
	{
	...
	//圆和旋转矩形碰撞
	public static bool CollideCircleAndRectRotate(GameVector2 _c1, int _radius1, GameVector2 _rCenter, int _half_w, int _half_h , int _dir)
	{
		//Debug.Log("CollideCircleAndRectRotate  *** 00  ");

	//	Debug.Log("前 " + _rCenter);

		float X = _rCenter.x * Mathf.Cos( changeDir(_dir))   + _rCenter.y * Mathf.Sin(changeDir(_dir));

		float Y = -_rCenter.x * Mathf.Sin(changeDir(_dir)) + _rCenter.y * Mathf.Cos(changeDir(_dir));
		...
		//Debug.Log("后 " + _rCenter);
		X = _c1.x * Mathf.Cos(changeDir(_dir)) + _c1.y * Mathf.Sin(changeDir(_dir));

		  Y = -_c1.x * Mathf.Sin(changeDir(_dir)) + _c1.y * Mathf.Cos(changeDir(_dir));
	...
	static float changeDir(float dir)
	{
		return (Mathf.PI * dir / 180.0f);
	}

	// 圆形和旋转矩形碰撞， 带修正
	public static bool CollideCircleAndRectRotate(GameVector2 _c1, int _radius1, GameVector2 _rCenter, int _half_w, int _half_h, out GameVector2 _amend, int _dir)
	{
		...
		*/
		...
		if (_amend != GameVector2.zero)
		{
			  X = _amend.x * Mathf.Cos(changeDir(-_dir)  ) + _amend.y * Mathf.Sin(changeDir(-_dir));

			  Y = -_amend.x * Mathf.Sin(changeDir(-_dir)) +_amend.y * Mathf.Cos(changeDir(-_dir));
